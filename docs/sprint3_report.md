...
## Как запустить демо
1. `cd server && npm install && npm run build && DEBUG=1 npm run start`
   (DEBUG=1 включает `/debug/*`-ручки для тестов; без него — чистый прод-режим).
2. Открыть `http://localhost:2567/` в **двух окнах** → «В бой · пушка/пулемёт».
   Соло-окно через 5 с добивается тремя ботами (DoD c); FFA 120 с, счёт — фраги.
3. Godot-клиент: запуск проекта с аргументами
   `-- --net=ws://127.0.0.1:2567/godot-relay` (web-экспорт — тот же URL;
   без аргумента — офлайн-соло Sprint 2).
